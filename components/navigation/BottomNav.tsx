...
export function BottomNav({ currentView, onChange }: BottomNavProps) {
  return (
    <nav className="fixed inset-x-0 bottom-0 z-20 px-2 pb-[calc(0.45rem+env(safe-area-inset-bottom))] sm:px-3 sm:pb-[calc(0.75rem+env(safe-area-inset-bottom))]">
      <div className="mx-auto grid max-w-4xl grid-cols-5 gap-1 rounded-[24px] border border-[var(--border)] bg-[rgba(20,13,51,0.82)] p-1.5 shadow-[0_18px_60px_rgba(5,3,18,0.46)] backdrop-blur-2xl">
        {items.map(({ icon: Icon, label, view }) => {
          ...
          return (
            <button
              ...
              className={`relative flex min-h-[3.35rem] min-w-0 flex-col items-center justify-center gap-0.5 rounded-[18px] px-0.5 text-[10px] leading-none transition active:scale-[0.98] sm:min-h-16 sm:gap-1 sm:text-xs ${
                active
            ...
            >
              ...
              <span className="whitespace-nowrap text-center font-medium">{label}</span>
            </button>
          );
        })}
      </div>
